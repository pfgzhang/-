...
public class GourmetCoffee  {
		...
		do  {
			try  {
				...
				stdErr.print(
					...
					+ "[10] Display the orders in JSON\n"
					+ "[11] Display the catalog in JSON\n"
					+"choice> ");
				stdErr.flush();
	...
	public void displayNumberOfOrders(Product product) {
		...
		for(Order order : sales) {
			if(order.getItem(product) != null) {
				if(order.getItem(product).getProduct() == product) {
					count += 1;
					}
			}
		}
	...
	public void displayTotalQuantityOfProducts() {
		...
		for(Product product : catalog) {
			...
			for(Order order : sales) {
				if(order.getItem(product) != null) {
					count += order.getItem(product).getQuantity();
				}
			}
	...
	private void displayorders() {
		String a = JSON.toJSONString(sales.getorders());
		System.out.println(a);
	}
	
	private void displaycatalog() {
		String a = JSON.toJSONString(catalog.getproducts());
		System.out.println(a);
	}
